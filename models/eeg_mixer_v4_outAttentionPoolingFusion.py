...
class AttentionPool1D(nn.Module):
    """
    Attention pooling over one token axis.
...
class TriAxisAttentionPoolingHead(nn.Module):
    ...
    It builds three token views:

        C-axis tokens: [B, C, K*T] -> [B, C, D]
        K-axis tokens: [B, K, C*T] -> [B, K, D]
        T-axis tokens: [B, T, C*K] -> [B, T, D]

    Then it performs attention pooling on each axis separately and
    combines the three pooled vectors by learnable weighted fusion.
    """
    def __init__(
        self,
        channel_dim: int,
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # C-axis tokens: each spatial latent component sees all K*T components.
        c_tokens = x.reshape(b, c, k * t)
        c_tokens = self.c_proj(c_tokens) + self.c_pos

        # K-axis tokens: each local temporal component sees all C*T components.
        k_tokens = x.permute(0, 2, 1, 3).contiguous().reshape(b, k, c * t)
        k_tokens = self.k_proj(k_tokens) + self.k_pos

        # T-axis tokens: each global temporal component sees all C*K components.
        t_tokens = x.permute(0, 3, 1, 2).contiguous().reshape(b, t, c * k)
        t_tokens = self.t_proj(t_tokens) + self.t_pos

        h_c = self.c_pool(c_tokens)  # [B, D]
        h_k = self.k_pool(k_tokens)  # [B, D]
        h_t = self.t_pool(t_tokens)  # [B, D]
...
class BasisMixer(nn.Module):
    def __init__(self, configs):
        ...
        # Basic configuration
        # -------------------------
        # seq_len is optional because patch number is inferred dynamically in _patchify().
        self.seq_len = int(_get_config(configs, "seq_len", None))
        ...
        # This is now used as the readout embedding dimension.
        ...
        if self.use_channel_adapter:
            self.channel_adapter = ChannelAdapter(
                ...
                residual_scale_init=float(_get_config(configs, "channel_adapter_residual_scale", 0.10)),
        ...
        #     raise ValueError(
        #         f"channel_basis_dim={self.channel_basis_dim} cannot exceed "
        #         f"active channel count={active_channels}"
        #     )
    ...
    def _patchify(
        ...
        """
        Convert continuous EEG into patchified 4D tensor.
    ...
    def _make_patch_embeddings(self, x: torch.Tensor) -> torch.Tensor:
        """
        Optional representation output.

        Input:
            x: [B, C, K, T]

        Output:
            patch_embeddings: [B, T, C]

        This is only used when output_mode='patch' or output_mode='both'.
        Classification uses the tri-axis attention pooling readout instead.
        """
    ...
    def forward(
        ...
        # Optional montage/channel adapter.
