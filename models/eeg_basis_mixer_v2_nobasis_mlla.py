...
class AxisAttention(nn.Module):
    """
    Mamba-like axis-wise linear mixer.

    This is a drop-in replacement for the original softmax AxisAttention.
    It keeps the same forward signature:

        forward(x, attend_dim, embed_axis)

    Input x is always [B, C, K, T].
    Internally, one axis is treated as sequence length and another axis is
    treated as embedding dimension, exactly like the original AxisAttention.

    Compared with standard softmax attention, this module uses:
    - LayerNorm over embedding dimension;
    - depthwise convolution along the sequence axis;
    - positive-kernel linear attention;
    - sigmoid gating;
    - output projection.

    It is not an exact Mamba SSM implementation, but it is a lightweight
    Mamba-like / linear-attention sequence mixer.
    """
    ...
    def __init__(
        ...
        # q, k, v, gate
        self.qkvz_proj = nn.Linear(self.embed_dim, self.embed_dim * 4)

        # Mamba-like local sequence mixing before global linear mixing.
        if conv_kernel % 2 == 0:
            conv_kernel += 1
    ...
    def _linear_mixer(self, x: torch.Tensor) -> torch.Tensor:
        """
        ...
        q, k, v, z = self.qkvz_proj(x).chunk(4, dim=-1)

        # Depthwise conv along sequence axis.
        # v: [B, L, D] -> [B, D, L] -> [B, L, D]
        ...
        # Non-causal global linear attention:
        # KV: [B, H, Dh, Dh]
