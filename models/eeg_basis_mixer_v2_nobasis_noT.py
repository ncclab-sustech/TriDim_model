...
class TriAxisMixerBlock_noT(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        return x + weights[0] * channel_branch + weights[1] * k_branch
...
class TriAxisEncoder(nn.Module):
    def __init__(self, channel_dim: int, k_dim: int, t_dim: int, n_layers: int, n_heads: int = 4, dropout: float = 0.0):
        super().__init__()
        self.layers = nn.ModuleList([TriAxisMixerBlock_noT(channel_dim, k_dim, t_dim, n_heads=n_heads, dropout=dropout) for _ in range(n_layers)])
