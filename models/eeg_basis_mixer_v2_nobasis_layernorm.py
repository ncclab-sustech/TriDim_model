...
class AxisLayerNorm(nn.Module):
    def __init__(self, dim_size: int, eps: float = 1e-5):
        super().__init__()
        self.dim_size = int(dim_size)
        self.norm = nn.LayerNorm(self.dim_size, eps=eps)

    def forward(self, x: torch.Tensor, dim: int) -> torch.Tensor:
        # x: [B, C, K, T]
        # Move the selected axis to the last dimension,
        # apply LayerNorm over that axis, then move it back.
        x_perm = x.transpose(dim, -1)
        x_norm = self.norm(x_perm)
        return x_norm.transpose(dim, -1)
...
class AxisAttention(nn.Module):
    def __init__(self, embed_dim: int, n_heads: int = 4, dropout: float = 0.0):
        ...
        # Transformer-style pre-norm over token embedding dimension
    ...
    def _attention(self, x: torch.Tensor) -> torch.Tensor:
        # x: [batch_like, seq_len, embed_dim]
        x = self.norm(x)
        ...
        qkv = self.qkv_proj(x)
        qkv = qkv.reshape(
            batch, seq_len, 3, self.n_heads, self.head_dim
        ).permute(2, 0, 3, 1, 4)
...
class TriAxisMixerBlock(nn.Module):
    def __init__(
        ...
        self.norm_c = AxisLayerNorm(channel_dim)
        self.norm_k = AxisLayerNorm(k_dim)
        self.norm_t = AxisLayerNorm(t_dim)
        ...
        # Optional but useful for stable deep stacking
        self.layer_scale = nn.Parameter(
            layer_scale_init * torch.ones(1)
        )

        self.out_drop = nn.Dropout(dropout)

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        # Branch-specific pre-norm
        x_c = self.norm_c(x, dim=1)
        x_k = self.norm_k(x, dim=2)
        x_t = self.norm_t(x, dim=3)

        # Own-axis nonlinear transformation
        channel_branch = self.channel_mlp(x_c, dim=1)
        k_branch = self.k_mlp(x_k, dim=2)
        t_branch = self.t_mlp(x_t, dim=3)

        # Axis-wise self-attention on the other two axes
        channel_branch = 0.5 * (
            self.attn_k_for_c(channel_branch, attend_dim=2, embed_axis=1)
            + self.attn_t_for_c(channel_branch, attend_dim=3, embed_axis=1)
        )

        k_branch = 0.5 * (
            self.attn_c_for_k(k_branch, attend_dim=1, embed_axis=2)
            + self.attn_t_for_k(k_branch, attend_dim=3, embed_axis=2)
        )

        t_branch = 0.5 * (
            self.attn_c_for_t(t_branch, attend_dim=1, embed_axis=3)
            + self.attn_k_for_t(t_branch, attend_dim=2, embed_axis=3)
        )

        weights = torch.softmax(self.fusion_logits, dim=0)

        out = (
            weights[0] * channel_branch
            + weights[1] * k_branch
            + weights[2] * t_branch
        )

        return x + self.out_drop(self.layer_scale * out)
