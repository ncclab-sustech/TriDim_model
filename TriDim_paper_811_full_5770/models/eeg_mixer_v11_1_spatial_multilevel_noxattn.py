...
VARIANT: noxattn — w/o Cross-Axis Attention (block-level ablation)
------------------------------------------------------------------
This variant validates the NECESSITY of cross-axis interaction: the entire
attention path is removed from TriAxisMixerBlock (the three shared
AxisAttention modules, the per-axis attention RMSNorms, the attention
fusion logits, gamma_attn, the per-branch DSS DropPaths, and the training-
only axis-branch dropout). What remains is a purely feed-forward tri-axis
mixer: each block is just the PreNorm tri-axis MLP path (identical to the
base model, byte-for-byte) with a single residual and LayerScale:

    x = x + gamma_mlp * _mlp_branch(x)

Everything else (frontend, projections, encoder, readout, config keys) is
identical to the base model; the same full YAML runs unchanged (only the
model name differs). drop_path_c/k/t and axis_branch_drop_* config values
are accepted for signature compatibility but have no effect.
...
class TriAxisMixerBlock(nn.Module):
    """
    V11 PreNorm + residual + tri-axis MLP + per-channel LayerScale.

    VARIANT noxattn: the cross-axis attention path is entirely removed
    (no AxisAttention modules, no attention RMSNorms, no attention fusion
    logits, no gamma_attn, no per-branch DSS DropPaths, no axis-branch
    dropout). The block keeps only the MLP path, identical to base:
    ...
    Differences from base:
        * Attention path removed; drop_path_c/k/t and axis_branch_drop_*
    ...
    def __init__(
        ...
        # ---- Per-axis RMSNorm: MLP path only (attention norms removed) ----
        ...
        # ---- Soft fusion logits (MLP path only) ----
    ...
    # -------------------------------------------------------------------------
    # Forward: single residual (attention residual removed in noxattn)
    # -------------------------------------------------------------------------
