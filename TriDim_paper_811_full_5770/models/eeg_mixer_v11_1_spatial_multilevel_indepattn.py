...
V5 backbone (untouched):
    ...
    * independent cross-axis attention (6 modules, one per view x attend pair)
    * per-channel LayerScale gamma on attn and mlp residual paths


ABLATION VARIANT: indepattn (Independent Attention Parameters)
================================================================

    This variant tests whether sharing each axis's attention parameters
    across its two attend directions (the base design) can preserve
    performance while reducing parameters.

    Base   : 3 shared AxisAttention modules per block -- attn_for_c is
             invoked for both attend=K and attend=T (K, T likewise), so
             each of the 3 modules is used twice.
    Variant: 6 independent AxisAttention modules per block -- the C view
             uses attn_c_attk + attn_c_attt, the K view uses attn_k_attc
             + attn_k_attt, the T view uses attn_t_attc + attn_t_attk.
             More parameters than the base.

    Everything else (per-axis RMSNorm, 0.5 averaging, DSS DropPath,
    softmax fusion, LayerScale, the MLP path) is byte-identical to the
    base file eeg_mixer_v11_1_spatial_multilevel.py.
...
class TriAxisMixerBlock(nn.Module):
    """
    V11 PreNorm + dual-residual + independent cross-axis attention
    ...
    Structure:
        ...
        c_out = dp_c(0.5 * (attn_c_attk(norm_c, attend=K) + attn_c_attt(norm_c, attend=T)))
        k_out = dp_k(0.5 * (attn_k_attc(norm_k, attend=C) + attn_k_attt(norm_k, attend=T)))
        t_out = dp_t(0.5 * (attn_t_attc(norm_t, attend=C) + attn_t_attk(norm_t, attend=K)))
    ...
    def __init__(
        ...
        # ---- Independent cross-axis attention: 6 modules, one per view x attend pair ----
        self.attn_c_attk = AxisAttention(self.channel_dim, n_heads=n_heads, dropout=dropout)
        self.attn_c_attt = AxisAttention(self.channel_dim, n_heads=n_heads, dropout=dropout)
        self.attn_k_attc = AxisAttention(self.k_dim,       n_heads=n_heads, dropout=dropout)
        self.attn_k_attt = AxisAttention(self.k_dim,       n_heads=n_heads, dropout=dropout)
        self.attn_t_attc = AxisAttention(self.t_dim,       n_heads=n_heads, dropout=dropout)
        self.attn_t_attk = AxisAttention(self.t_dim,       n_heads=n_heads, dropout=dropout)
    ...
    def _attn_branch(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # C-view: norm along C, attend along K and along T (independent params).
        x_c = self.norm_c_attn(x)
        c_out = 0.5 * (
            self.attn_c_attk(x_c, attend_dim=2, embed_axis=1)
            + self.attn_c_attt(x_c, attend_dim=3, embed_axis=1)
        )
        ...
        k_out = 0.5 * (
            self.attn_k_attc(x_k, attend_dim=1, embed_axis=2)
            + self.attn_k_attt(x_k, attend_dim=3, embed_axis=2)
        )
        ...
        t_out = 0.5 * (
            self.attn_t_attc(x_t, attend_dim=1, embed_axis=3)
            + self.attn_t_attk(x_t, attend_dim=2, embed_axis=3)
        )
