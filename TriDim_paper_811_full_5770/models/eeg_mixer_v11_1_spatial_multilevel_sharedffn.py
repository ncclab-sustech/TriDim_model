...
=============================================================================
ABLATION VARIANT: sharedffn (Shared FFN)
=============================================================================

This variant ablates the axis-specific FFN transforms of the TriDim block.
It tests whether giving each axis its own MLP weights (channel_mlp / k_mlp /
t_mlp in the base model) is actually necessary, or whether a single shared
FFN applied to all three axis views is sufficient.

Change relative to the base model (everything else is byte-identical):

    * The three independent AxisMLP modules (channel_mlp, k_mlp, t_mlp)
      are replaced by ONE shared MLP module (`shared_mlp`), structurally
      identical to AxisMLP (two-layer MLP, expansion=2, GELU + Dropout),
      but built on the unified channel_dim width.

    * Zero-padding scheme (implementation detail reported in the paper):
      each axis view is RMSNorm-ed along its own axis as in the base, then
      the target axis is transposed to the last dimension. If that axis
      size is smaller than channel_dim, the last dimension is zero-padded
      on the right up to channel_dim; the shared MLP is applied; the output
      is sliced back to the original axis size and transposed back. The
      C-axis view (size == channel_dim) goes through the shared MLP
      directly without padding. All paper configs use
      channel_basis_dim >= k_basis_dim, t_basis_dim (e.g. 32 vs 16/16),
      so padding only ever widens K / T views; no view is ever truncated.

    * mlp_fusion_logits, dp_mlp, gamma_mlp and the softmax fusion of the
      three axis views are kept exactly as in the base model.

    * The attention path is byte-identical to the base model.

Consequence: the nn.Module tree contains exactly ONE MLP-class module
(shared_mlp) instead of three, and the MLP-path parameter count drops
accordingly (one channel_dim-width MLP instead of C + K + T width MLPs).
...
class TriAxisMixerBlock(nn.Module):
    ...
    Structure:
        ...
        # MLP path: 3 sub-branches in parallel sharing ONE FFN, single DropPath
        # at the end (sharedffn ablation: K / T views are zero-padded to
        # channel_dim before the shared FFN and sliced back afterwards).
        c_out = shared_mlp(norm_c_mlp, dim=1)
        k_out = shared_mlp(pad(norm_k_mlp), dim=2)[..., :k_dim]
        t_out = shared_mlp(pad(norm_t_mlp), dim=3)[..., :t_dim]
    ...
    def __init__(
        ...
        # ---- Shared axis-wise MLP (sharedffn ablation) ----
        # ONE MLP (AxisMLP structure, expansion=2) on the unified channel_dim
        # width, shared by all three axis views. Views whose axis is narrower
        # than channel_dim are zero-padded to channel_dim before this module
        # and sliced back afterwards (see _mlp_branch / _apply_shared_mlp).
        self.shared_mlp = AxisMLP(self.channel_dim, dropout=dropout)
    ...
    def _apply_shared_mlp(self, x: torch.Tensor, dim: int) -> torch.Tensor:
        """Apply the shared FFN along axis `dim` of a [B, C, K, T] tensor.

        Zero-padding scheme: the target axis is transposed to the last
        dimension; if its size is smaller than channel_dim, the last
        dimension is zero-padded on the right up to channel_dim; the shared
        MLP (channel_dim width) is applied; the output is sliced back to
        the original axis size and transposed back. Views already at
        channel_dim (the C view) skip padding entirely.
        """
        axis_dim = x.shape[dim]
        if axis_dim >= self.channel_dim:
            return self.shared_mlp(x, dim=dim)
        x_perm = x.transpose(dim, -1)
        x_perm = F.pad(x_perm, (0, self.channel_dim - axis_dim))
        out = self.shared_mlp(x_perm, dim=-1)
        out = out[..., :axis_dim]
        return out.transpose(dim, -1)
    ...
    def _mlp_branch(self, x: torch.Tensor) -> torch.Tensor:
        ...
        sharedffn ablation: all three axis views go through ONE shared MLP
        (channel_dim width); K / T views are zero-padded / sliced back.
        """
        x_c = self.norm_c_mlp(x)
        c_out = self._apply_shared_mlp(x_c, dim=1)

        x_k = self.norm_k_mlp(x)
        k_out = self._apply_shared_mlp(x_k, dim=2)

        x_t = self.norm_t_mlp(x)
        t_out = self._apply_shared_mlp(x_t, dim=3)
