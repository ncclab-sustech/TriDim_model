...
VARIANT: seqckt — Sequential C→K→T (block-level ablation)
----------------------------------------------------------
This variant validates whether the PARALLEL tri-axis structure (with
learned softmax fusion over the three axis views) is better than a FIXED
processing order: inside each block, the three axis views are applied
sequentially in the fixed order C → K → T, each view reading the tensor
updated by the previous one:

    # Attention path: 3 sequential sub-updates sharing gamma_attn
    x = x + gamma_attn * dp_c_attn(0.5 * (attn_for_c(norm_c_attn(x), attend=K)
                                        + attn_for_c(norm_c_attn(x), attend=T)))
    x = x + gamma_attn * dp_k_attn(...)   # K-view on the C-updated x
    x = x + gamma_attn * dp_t_attn(...)   # T-view on the K-updated x

    # MLP path: 3 sequential sub-updates sharing gamma_mlp and dp_mlp
    x = x + gamma_mlp * dp_mlp(channel_mlp(norm_c_mlp(x), dim=1))
    x = x + gamma_mlp * dp_mlp(k_mlp(norm_k_mlp(x), dim=2))
    x = x + gamma_mlp * dp_mlp(t_mlp(norm_t_mlp(x), dim=3))

The attn_fusion_logits / mlp_fusion_logits parameters are removed (no
fusion is needed in the sequential form); the parameter count drops by
6 scalar weights per block, which is negligible. The training-only axis
branch dropout keeps its semantics via a sequential analogue: with the
configured probability, one of the three attention sub-updates is skipped
per batch and the surviving two are scaled by 1.5 so the per-update
expectation is preserved.

Everything else (frontend, projections, encoder, readout, config keys,
DSS rates, LayerScale) is identical to the base model; the same full YAML
runs unchanged (only the model name differs).
...
class TriAxisMixerBlock(nn.Module):
    ...
    VARIANT seqckt: the three axis views are applied SEQUENTIALLY in the
    fixed order C → K → T instead of in parallel with softmax fusion.
    Each view reads the tensor updated by the previous view; all three
    attention sub-updates share gamma_attn, and all three MLP sub-updates
    share gamma_mlp and the single dp_mlp DropPath module.

    Structure:

        # Attention path: 3 sequential sub-updates, each with its own DropPath
        x = x + 污_attn × dp_c(0.5 * (attn_for_c(norm_c(x), attend=K) + attn_for_c(norm_c(x), attend=T)))
        x = x + 污_attn × dp_k(0.5 * (attn_for_k(norm_k(x), attend=C) + attn_for_k(norm_k(x), attend=T)))
        x = x + 污_attn × dp_t(0.5 * (attn_for_t(norm_t(x), attend=C) + attn_for_t(norm_t(x), attend=K)))

        # MLP path: 3 sequential sub-updates, single shared DropPath
        x = x + 污_mlp × dp_mlp(channel_mlp(norm_c_mlp(x), dim=1))
        x = x + 污_mlp × dp_mlp(k_mlp(norm_k_mlp(x), dim=2))
        x = x + 污_mlp × dp_mlp(t_mlp(norm_t_mlp(x), dim=3))

    No attn_fusion_logits / mlp_fusion_logits: fusion is meaningless in
    the sequential form (parameter count drops by 6 scalars, negligible).

    Differences from V5:
        * Each attention sub-update is gated by its own DropPath rate (DSS).
          Default: drop_path_c=0.25, drop_path_k=0.05, drop_path_t=0.15.
        * The MLP path is gated by a single DropPath rate (drop_path_mlp),
          shared by all three sequential sub-updates.
    ...
    def __init__(
        ...
        # ---- Soft fusion logits REMOVED in this variant ----
        # Sequential C→K→T needs no cross-view softmax fusion; the base
        # model's attn_fusion_logits / mlp_fusion_logits (6 scalar weights
        # per block) are dropped, a negligible parameter-count change.
        ...
        # Attention path: one DropPath per sub-update (C / K / T).
        self.dp_c_attn = DropPath(drop_path_c)
        self.dp_k_attn = DropPath(drop_path_k)
        self.dp_t_attn = DropPath(drop_path_t)
        # MLP path: a single DropPath module, shared by all three
        # sequential axis-MLP sub-updates.
        self.dp_mlp    = DropPath(drop_path_mlp)

        # Optional training-only axis branch dropout (sequential analogue):
        # with random_one, one attention axis sub-update is skipped per
        # batch and the surviving two are scaled by 1.5 so the per-update
        # expectation is preserved. Default 0 is no-op.
    ...
    # -------------------------------------------------------------------------
    # Branch computations (sequential C -> K -> T)
    # -------------------------------------------------------------------------

    def _attn_branch(self, x: torch.Tensor) -> torch.Tensor:
        """Sequential C -> K -> T cross-axis attention updates (DSS per axis).

        Each axis view reads the tensor updated by the previous view and
        adds its own residual; all three sub-updates share gamma_attn.
        """
        # Optional training-only axis branch dropout (sequential analogue of
        # the base model's parallel branch masking): skip one sub-update per
        # batch, scale the surviving two by 1.5 to preserve expectation.
        drop_idx = -1
        scale = 1.0
        ...
        if drop_idx != 0:
            ...
            x = x + scale * self.gamma_attn * self.dp_c_attn(c_out)

        # K-view: applied on the C-updated x.
        if drop_idx != 1:
            ...
            x = x + scale * self.gamma_attn * self.dp_k_attn(k_out)

        # T-view: applied on the K-updated x.
        if drop_idx != 2:
            ...
            x = x + scale * self.gamma_attn * self.dp_t_attn(t_out)

        return x

    def _mlp_branch(self, x: torch.Tensor) -> torch.Tensor:
        """Sequential C -> K -> T axis-MLP updates, dp_mlp shared by all three."""
        x = x + self.gamma_mlp * self.dp_mlp(self.channel_mlp(self.norm_c_mlp(x), dim=1))
        x = x + self.gamma_mlp * self.dp_mlp(self.k_mlp(self.norm_k_mlp(x), dim=2))
        x = x + self.gamma_mlp * self.dp_mlp(self.t_mlp(self.norm_t_mlp(x), dim=3))
        return x

    # -------------------------------------------------------------------------
    # Forward: attention path then MLP path (each sequential C -> K -> T)
    # -------------------------------------------------------------------------

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        x = self._attn_branch(x)
